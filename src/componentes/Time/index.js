...
const Time = ({ time, colaboradores,aoDeletar,mudarCores }) => {
    return (
        ...
        colaboradores.length > 0 && <section className='time' style={{ backgroundImage: 'url(/imagens/fundo.png)', backgroundColor: hexToRgba(time.cor, '0.6') }}>
            ...
            <input onChange={evento => mudarCores(evento.target.value, time.nome)} value={time.cor} type='color' className='input-cor'/>
